...
def load_and_prepare(csv_files):
    ...
    for f in csv_files:
        ...
        participant = os.path.splitext(os.path.basename(f))[0]  # Extract 'user1' from 'user1.csv'
...
def fit_mixed_effects(data, psqi_path='../data/psqi_scores.csv'):
    ...
    if df.empty:
        print("[ERROR] No matching participant + date found.")
        return None
